...
export default class StateMachine<S, T, D> {
	...
	/***
	 * Returns JSON serialized state for native usage JSON.stringify
	 *
	 * @example
	 * const stateMachine = new StateMachine(...);
	 * //... some stateMachine changes
	 * const jsonState = JSON.stringify(stateMachine); // "{\"state\":\"GAS\",\"data\":{\"deg\":200},\"transport\":{\"some\":\"some\"}}"
	 */
	public toJSON(): IHydratedState<S, D> {
		return {
			state: this.state,
			data: this.data,
	...
	/**
	 * Returns string representing of state machine
	 *
	 * @example
	 * const stateMachine = new StateMachine(...);
	 * //... some stateMachine changes
	 * String(stateMachine) // "StateMachine(SOLID){\"state\":\"SOLID\",\"data\":-100,\"transport\":{}}"
	 * String(stateMachine) // "StateMachine(SOLID){\"state\":\"SOLID\",\"data\":-100,\"transport\":{}}"
	 */
	public toString(): string {
		return `${this.constructor.name}(${this.state})${JSON.stringify(this)}`;
	}
	...
	public async transitTo(
		...
		...args: any[]
	): Promise<IStateMachineTransitionResult> {
	...
	private async _checkAndDoTransition(
		...
		if (!currentTransition) {
			...
			return { isSuccess: false, error: this._onError(error) };
		}
	...
	private async _transit(
		...
		let transitionResult: { isSuccess: false } = { isSuccess: false };
		...
		const isOkAfterEachStateHook = await this._runHooks(
		...
		if (isOkAfterEachStateHook === false) {
			return {
				...transitionResult,
				// error: new StateMachineError("", StateMachineErrorCode.ABSENT_PROPERTY),
			};
		}
	 ...
	 * Return StateMachineError otherwise.
	 */
	private _onError(error: StateMachineError): never | StateMachineError /*| void */ {
	...
	private async _runHooks(
		...
		transition: _ITransition<S, T, D>,
		args: any[] = [],
	): Promise<boolean | IStateMachineTransitionResult> {
		// console.log(args);
		const hooksResult = await hooks.reduce<Thenable<void | false | IStateMachineTransitionResult>>(async (acc, callback) => {
			...
			if (accResult === false) {
				...
				// 	)
				// );
				let timeoutPromise: Promise<IStateMachineTransitionResult>;
					...
					timeoutPromise = new Promise((_resolve, reject) => {
						const timer = setTimeout(() => {
							...
							try {
								...
								reject(this._onError(timeoutError));
							} catch (error) {
								reject(error);
							}
						}, this[CONFIG].timeout);
				...
				return Promise.race([handlerResult, timeoutPromise]).catch(error => ({
					isSuccess: false,
					error: error,
				}));
			}
